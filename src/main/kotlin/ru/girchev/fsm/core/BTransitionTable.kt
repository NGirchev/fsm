...
internal constructor(
    ...
    open class Builder<STATE> {
        ...
        fun add(from: STATE, vararg to: STATE): Builder<STATE> {
            val list: List<BTransition<STATE>> = to.map { BTransition(from, it) }
        ...
        fun add(from: STATE, vararg to: To<STATE>): Builder<STATE> {
            for (t in to) {
                transitions.putIfAbsent(from, LinkedHashSet())
                val transition = BTransition(from, t.to, t.condition, t.action, t.timeout)
        ...
        open fun build(): BTransitionTable<STATE> {
